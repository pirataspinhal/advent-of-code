...
someFunc = do
  ...
  print $ countValid allFields
  putStr "Passports with valid fields "
  print $ countValid validated
  where
    countValid list = length $ filter (== True) list
    checkWith parser = map (checkPassport parser) inputs
    allFields = checkWith existanceParser
    validated = checkWith validationParser
